...
class Trainer():
    ...
    def __init__(self, model, train_loader, validation_loader, learning_rate, num_epoch, model_path, load_model):
        self.model = model
        ...
        print(Fore.BLUE + f'Using {self.device}' + Style.RESET_ALL)  # my ubuntu doesn't see the graphics (maybe a driver problem)   

        # Setup matplotlib figure
        plt.title('Training', fontweight="bold")
        plt.axis([0, self.num_epoch, 0, 10])
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        self.handle = None
    ...
    def train(self):
      ...
      # Resume model or start from scratch
                ...
                for state in self.optimizer.state.values():
                    for k, v in state.items():
                        if isinstance(v, torch.Tensor):
                            state[k] = v.cuda() # Desbloquear se a placa gráfica estiver a funcionar
        ...
        for epoch_idx in range(start_epoch, self.num_epoch):
            ...
            for batch_idx, (inputs, labels_gt) in tqdm(enumerate(self.train_loader),
                                                       ...
                                                       desc= f'Training batches for epoch idx {epoch_idx} '):
                ...
                # print(batch_loss.data.item())

                # Update Model
                self.optimizer.zero_grad() # resets the gradients form previous batches
                batch_loss.backward()
                self.optimizer.step()
                ...
                # print(f'Batch idx {batch_idx} loss = {batch_loss}')
